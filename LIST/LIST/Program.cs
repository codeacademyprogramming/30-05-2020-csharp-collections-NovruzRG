...
namespace LIST
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            RtyhaOrDayrep(debtors);
            From26To36(debtors);
            DebtLessThan5k(debtors);
            Less18Symbol(debtors);
            Winter(debtors);
            AverageDebt(debtors);
            Not8(debtors);
            NameSurname3samechar(debtors);
            MostlyYear(debtors);
            MaxFiveDebt(debtors);
            TotalDebt(debtors);
            WorldWar2nd(debtors);
            Pay500Monthly(debtors);
        ...
        static void RtyhaOrDayrep(List<Debtor> debtors)
        {
            var result = debtors.Where(x => x.Email.Contains("rhyta.com") || x.Email.Contains("dayrep.com")).ToList();
            Result(result);
        }
        static void DebtLessThan5k(List<Debtor> debtors)
        {
            var result = debtors.Where(x => x.Debt < 5000).ToList();
            Result(result);
        }
        static void TotalDebt(List<Debtor> debtors)
        {
            int totalDebt = 0;
            for (int i = 0; i < debtors.Count; i++)
            {
                totalDebt += debtors[i].Debt;
            }
            Console.WriteLine($"Total Debt all Debtors is -- {totalDebt}");
        }
        static void AverageDebt(List<Debtor> debtors)
        {
            int totalDebt = 0;
            for (int i = 0; i < debtors.Count; i++)
            {
            ...
            decimal AverageDebt = totalDebt / debtors.Count;
            var result = debtors.Where(x => x.Debt > AverageDebt).ToList();
            result = result.OrderBy(x => x.FullName).OrderBy(x => x.Debt).Reverse().ToList();
            Result(result);
        }
        static void MaxFiveDebt(List<Debtor> debtors)
        {
            var result = debtors.OrderBy(x => x.Debt).Reverse().Take(5).ToList();
            Result(result);
        }
        static void Winter(List<Debtor> debtors)
        {
            var result = debtors.Where(x => x.BirthDay.Month == 12 || x.BirthDay.Month == 1 || x.BirthDay.Month == 2).ToList();
            Result(result);
        }
        static void WorldWar2nd(List<Debtor> debtors)
        {
            DateTime time = new DateTime(1945, 9, 2);
            var result = debtors.Where(x => x.BirthDay <= time).ToList();
            Result(result);
        }
        static void From26To36(List<Debtor> debtors)
        {
            ...
            {
                var age = 0;
                if (time.Month > debtors[i].BirthDay.Month)
                {
                    age = time.Year - debtors[i].BirthDay.Year;
                }
                else if (time.Month == debtors[i].BirthDay.Month)
                {
                    if (time.Day >= debtors[i].BirthDay.Day)
                    {
                        age = time.Year - debtors[i].BirthDay.Year;
                    }
                    else
                    {
                        age = time.Year - debtors[i].BirthDay.Year - 1;
                    }
                }
                else
                {
        ...
        static void Pay500Monthly(List<Debtor> debtors)
        {
            DateTime time = DateTime.Today;
            for (int i = 0; i < debtors.Count; i++)
            {
                ...
                if (time.Month > debtors[i].BirthDay.Month)
                {
                    month = 12 - time.Month + debtors[i].BirthDay.Month;
                }
                else
                {
                    month = debtors[i].BirthDay.Month - time.Month;
                }
                if (month * 500 >= debtors[i].Debt)
                {
                    Console.WriteLine(debtors[i]);
                }
            }
        ...
        static void Not8(List<Debtor> debtors)
        {
            var result = debtors.Where(x => !x.Phone.Contains("8")).ToList();
            ResultWithAge(result);
        }
        static void Less18Symbol(List<Debtor> debtors)
        {
            var result = debtors.Where(x => x.FullName.Length > 18 && x.Phone.ToCharArray().Count(f => f == '7') >= 2).ToList();
            Result(result);
        }
        static void MostlyYear(List<Debtor> debtors)
        {

            Dictionary<int, int> MostlyYear = new Dictionary<int, int>();
            for (int i = 0; i < debtors.Count; i++)
            {
                if (MostlyYear.ContainsKey(debtors[i].BirthDay.Year))
                {
                    MostlyYear[debtors[i].BirthDay.Year]++;
                }
                else
                {
                    MostlyYear.Add(debtors[i].BirthDay.Year, 1);
                }
            }
            List<int> years = MostlyYear.Values.ToList();
            int maxYear = years.Max();

            foreach (var item in MostlyYear)
            {
                if (item.Value == maxYear)
                {
                    Console.WriteLine($"From {item.Key} - {item.Value} Debtors");
                }
            }
        }
        static void NameSurname3samechar(List<Debtor> debtors)
        {
            List<Debtor> samechar = new List<Debtor>();
            for (int i = 0; i < debtors.Count(); i++)
            {
                Dictionary<char, int> pairs = new Dictionary<char, int>();
                foreach (var item in debtors[i].FullName)
                {
                    ...
                    if (pairs[item] == 3)
                    {
                        samechar.Add(debtors[i]);
                        break;
                    }
                }
            }
            samechar = samechar.OrderBy(x => x.FullName).ToList();
            Result(samechar);
        }

        static void Result(List<Debtor> list)
        {
        ...
        static void ResultWithAge(List<Debtor> list)
        {
            foreach (var item in list)
            {
                DateTime time = DateTime.Today;
                var age = 0;
                if (time.Month > item.BirthDay.Month)
                {
                    age = time.Year - item.BirthDay.Year;
                }
                else if (time.Month == item.BirthDay.Month)
                {
                    if (time.Day >= item.BirthDay.Day)
                    {
                        age = time.Year - item.BirthDay.Year;
                    }
                    else
                    {
                        age = time.Year - item.BirthDay.Year - 1;
                    }
                }
                else
                {
                ...
                Console.WriteLine(item.FullName + "  " + age + "  " + item.Debt);
            }
            Console.WriteLine("---------------------------");
        }
    }
